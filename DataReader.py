...
class DataReader:
    ...
    def read_dat_file(self):
        ...
        file_list = os.listdir(self.directory)
        dat_file = [filename for filename in file_list if filename.endswith('.dat')][0]
    ...
    def load_data(self,file_path):
        ...
        df = df[~((df['bin_index'] == df['bin_index'].max()) & (df.drop(['timestep', 'bin_index', 'coord'], axis=1) == 0).all(axis=1))]
