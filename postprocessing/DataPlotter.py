...
class DataPlotter:
    ...
    def plot_time_variation(self, results_dict, df):
        ...
        for i, variable in enumerate(pressure_variables):
            if variable in df.columns and variable in results_dict.keys():
                ...
                plt.plot(
                    strain, df[variable], label=f'${variable}$ (time variation)', linestyle='-', linewidth=1, color=color)
                plt.plot(
                    strain, mean, label=f'${variable}$ (average)', linestyle='--', linewidth=2, color=color)
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(self.directory + 'pressure_variables_alpha_' +
                    self.ap + '_cof_' + self.cof + '_I_' + self.value + '.png')

        # Plot other variables in separate plots
        for i, variable in enumerate(other_variables):
            if variable in df.columns and variable in results_dict.keys():
                if variable == 'Omega_z':
                    results_dict[variable] = results_dict[variable] / \
                        (results_dict['max_vx_diff'] /
                         (2*results_dict['box_height']))
                ...
                plt.grid(True)
                plt.savefig(
                    f'{self.directory}{variable}_alpha_{self.ap}_cof_{self.cof}_I_{self.value}.png')
        ...
        if 'msdY' in df.columns or 'msdZ' in df.columns:
            ...
            if 'msdY' in df.columns:
                plt.plot(strain, df['msdY'], label='msdY (time variation)',
                         linestyle='-', linewidth=1, color='magenta')

            # Plot msdZ if available
            if 'msdZ' in df.columns:
                plt.plot(strain, df['msdZ'], label='msdZ (time variation)',
            ...
            # plt.show()
            # Save the figure
            plt.savefig(self.directory + 'msdYZ_alpha_' + self.ap +
                        '_cof_' + self.cof + '_I_' + self.value + '.png')
            plt.close()

    def plot_variable(self, bins, averages, std_devs, variable):
        plt.errorbar(averages, bins / np.max(bins) - 0.5,
                     xerr=std_devs, fmt='-o', capsize=5, label=variable)
        plt.ylabel('Y/H (height)')
        plt.xlabel(f'{variable} (averaged)')
        plt.title(
            f'Average {variable} with Standard Deviation (excluding first 1/11th)')
        plt.legend()
        plt.grid(True)
        plt.savefig(
            f'{self.directory}{variable}_avg_std_dev_alpha_{self.ap}_cof_{self.cof}_I_{self.value}.png')
    ...
    def plot_averages_with_std(self, results_dict):
        ...
        for i, variable in enumerate(velocity_variables):
            ...
            if avg_key in results_dict and std_dev_key in results_dict:
                self.plot_variable_on_same_plot(
                    results_dict['bins'], results_dict[avg_key], results_dict[std_dev_key], variable, colormap[i])
        plt.ylabel('Y/H (height)')
        ...
        plt.grid(True)
        plt.savefig(
            f'{self.directory}velocity_avg_std_dev_alpha_{self.ap}_cof_{self.cof}_I_{self.value}.png')
        plt.close()
        # plt.show()
        ...
        plt.grid(True)
        plt.savefig(
            f'{self.directory}stress_avg_std_dev_alpha_{self.ap}_cof_{self.cof}_I_{self.value}.png')
        # plt.show()
        plt.close()
        ...
        if avg_key in results_dict and std_dev_key in results_dict:
            plt.figure(figsize=(10, 6))
            self.plot_variable(
                results_dict['bins'], results_dict[avg_key], results_dict[std_dev_key], variable)
    ...
    def plot_variable_on_same_plot(self, bins, averages, std_devs, variable, color):
        plt.errorbar(averages, bins / np.max(bins) - 0.5, xerr=std_devs,
                     fmt='-o', capsize=5, label=variable, color=color)
    ...
    def plot_histogram(self, bins, hist, variable, label='$\\theta_x [^\\circ]$'):
        ...
        # Plot the histogram
        plt.hist(bin_centers, bins=bins, weights=hist,
                 color='gray', alpha=0.7, edgecolor='k')

        # Highlight the bin with the median probability
        plt.hist([bin_centers[median_index]], bins=bins, weights=[
                 median_value], color='red', alpha=0.9, edgecolor='k')

        if label == '$\\theta_x [^\\circ]$':
            # Draw a vertical line at the angle corresponding to tan^-1(1/ap)
            plt.axvline(x=theta_ap, color='blue', linestyle='--',
                        linewidth=2, label=r'$\theta = \tan^{-1}(1/\alpha)$')

        # Set axis labels and title
        plt.xlabel(label)
        plt.ylabel('Probability')
        plt.title(
            f'2D Histogram of {variable} with Highlighted Median', fontsize=14)
        plt.legend()
        plt.savefig(
            f'{self.directory}histogram_{variable}_alpha_{self.ap}_cof_{self.cof}_I_{self.value}.png')
        # plt.show()
        plt.close()
    ...
    def plot_histogram_ellipsoid_flat(self, hist_flat, num_bins_theta, num_bins_phi,
        ...
        # u: azimuthal angle (0 to 2pi)
        u = np.linspace(0, 2 * np.pi, num_bins_phi + 1)
        # v: polar angle (0 to pi)
        v = np.linspace(0, np.pi, num_bins_theta + 1)
        ...
        ax.plot_surface(x, y, z, facecolors=plt.cm.inferno(
            colors), rstride=1, cstride=1, antialiased=True, alpha=1.0)
        ...
        plt.title(title)
        plt.savefig(
            f'{self.directory}ellipsoid_{title}_alpha_{self.ap}_cof_{self.cof}_I_{self.value}.png')
        # plt.show()
        plt.close()
    ...
    def plot_histogram_ellipsoid(self, hist_local, bins_local, title, label, colormap='YlGnBu'):
        ...
        # use bins_local length for polar angle divisions
        v = np.linspace(0, np.pi, 2 * len(bins_local)-1)
        ...
        ax.plot_surface(x, y, z, facecolors=cmap(norm(colors)),
                        rstride=1, cstride=1, antialiased=True, alpha=1.0)
    ...
    def plot_pdf(self, data, n_bins, variable, label='$\\theta_x [^\\circ]$', median_value=None):
        ...
        plt.hist(data, bins=bins, density=True, alpha=0.7,
                 color='gray', edgecolor='k', label='Orientation Data')
        # Highlight the bin with the median probability
        plt.axvline(x=median_value, color='red', linestyle='--',
                    linewidth=2, label=f'$\\theta_x^m$ = {median_value:.2f} rad')
    ...
    def plot_bar_histogram(self, bins, hist, variable):
        ...
        plt.imshow(data, aspect='equal', cmap='plasma',
    ...
    def plot_polar_histogram(self, bins, histogram, title, symmetry=False):
        ...
        if symmetry:
            ...
            assert len(histogram) == len(
                bins) - 1, "The length of histogram should match the number of bins - 1."
            ...
            extended_histogram = np.concatenate(
                (histogram, histogram_mirror_90_180, histogram, histogram_mirror_90_180))
        ...
        ax.bar(angles_rad, extended_histogram, width=bar_widths,
               bottom=0, color='b', alpha=0.6, edgecolor='k')

        # Set radius limit to the maximum histogram value
        # 1.1 for a little extra space above the tallest bar
        ax.set_ylim(0, np.max(extended_histogram) * 1.1)

        plt.savefig(
            f'{self.directory}polar_histogram_{title}_alpha_{self.ap}_cof_{self.cof}_I_{self.value}.png')
        # Display the plot
        # plt.show()
        plt.close()
    ...
    def plot_eulerian_velocities(self, data):
        ...
        for i in range(n_plots):
            ...
            color = plt.cm.viridis(i / n_plots)
            # plot the eulerian velocity at the given time steps skipping the first 2 strains
            if i != 0:
                plt.plot(data['eulerian_vx'][i*time_interval, :], y,
                         label=f'strain  = {i*time_interval*20/n_time_steps+2:.2f}', color=color)
        plt.xlabel('Vx/V')
        plt.ylabel('y/H')
        plt.legend()
        fig2.suptitle('ap = ' + self.ap + ', cof = ' + self.cof +
                      ', ' + self.parameter + '=' + self.value)
        fig2.savefig(self.directory + 'simple_shear_ap' + self.ap + '_cof_' +
                     self.cof + '_' + self.parameter + '_' + self.value + 'eulerian.png')
        plt.clf()
        # plt.show()
    ...
    def plot_force_distribution(self, force_normal_distribution, force_tangential_distribution):
        ...
        plt.figure()
        plt.subplot(2, 1, 1)
        plt.title('ap = ' + self.ap + ', cof = ' + self.cof +
                  ', ' + self.parameter + '=' + self.value)
        plt.plot(force_normal_distribution[1]
                 [:-1], force_normal_distribution[0])
        plt.xlabel('F_n')
        plt.ylabel('P(F_n)')
        plt.subplot(2, 1, 2)
        plt.plot(force_tangential_distribution[1]
                 [:-1], force_tangential_distribution[0])
        plt.xlabel('F_t')
        plt.ylabel('P(F_t)')
        plt.savefig(self.directory + 'simple_shear_ap' + self.ap + '_cof_' + self.cof +
                    '_' + self.parameter + '_' + self.value + 'force_distribution.png')
        plt.show()
    ...
    def plot_ellipsoids(self, step, data):
        ...
        ax.set_ylabel('Y ')
        ax.set_zlabel('Z ')
        ax.set_title('ap = ' + self.ap + ', cof = ' + self.cof + ', ' +
                     self.parameter + '=' + self.value + ', step='+str(step))
    ...
    def plot_space_averages_all_cells(self, value, quantity='Velocity', component=None, symbol='', nx_divisions=40, ny_divisions=6):
        ...
        if component is None:
            plt.imshow(value.T, cmap='plasma', origin='lower',
                       extent=[0, nx_divisions, 0, ny_divisions])
        else:
            plt.imshow(value[:, :, component].T, cmap='plasma', origin='lower', extent=[
                       0, nx_divisions, 0, ny_divisions])

        cbar = plt.colorbar(label=symbol, orientation='horizontal')
        # Set the label and increase the font size
        cbar.set_label(label=symbol, fontsize=36)
        # Adjust the font size of the color bar tick labels
        ...
        tick_labels = np.linspace(-self.box_x/2/self.h,
                                  self.box_x/2/self.h, num=15)
        ...
        plt.yticks(np.linspace(0, ny_divisions, num=5),
                   np.linspace(0, 1, num=5), fontsize=14)
    ...
    def plot_time_series_forces(self, time, fx, fy, fz):
        ...
        plt.ylabel('Force [N]')
        plt.legend()
        plt.title('fraction = ' + str(self.fraction) + ', ap = ' + str(self.ap) + ', cof = ' +
                  str(self.cof) + ', muw = ' + str(self.muw) + ', vwall = ' + str(self.vwall))
        plt.savefig("time_series_force_" + ', ap = ' + str(self.ap) + ', cof = ' + str(
            self.cof) + ', muw = ' + str(self.muw) + ', vwall = ' + str(self.vwall) + ".png")
        plt.show()
    ...
    def plot_time_series_energy(self, time, tke, rke):
        ...
        plt.title('fraction = ' + str(self.fraction) + ', ap = ' + str(self.ap) + ', cof = ' +
                  str(self.cof) + ', muw = ' + str(self.muw) + ', vwall = ' + str(self.vwall))
        plt.savefig("time_series_energy_" + ', ap = ' + str(self.ap) + ', cof = ' + str(
            self.cof) + ', muw = ' + str(self.muw) + ', vwall = ' + str(self.vwall) + ".png")
        plt.show()
