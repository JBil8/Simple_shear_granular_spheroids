...
class ProcessorDump(DataProcessor):
    ...
    def process_single_step(self, step, coor, orientation, shapex, shapez,
        ...
        global_normals = self.compute_normals(
            centers1, self.point, orientations1, shapex1, shapez1, self.force, self.shear)
        ...
        curv1 = self.compute_gaussian_curvature_coefficient(
            shapex1, shapez1, contact_vectors1, orientations1)
        curv2 = self.compute_gaussian_curvature_coefficient(
            shapex2, shapez2, contact_vectors2, orientations2)
        ...
        vel_correction1 = vel_correction2 = angular_correction1 = angular_correction2 = np.zeros_like(
            vel1)
        relative_velocities = self.compute_relative_velocity(vel1, vel2, omega1, omega2, contact_vectors1, contact_vectors2,
                                                             # relative velocity is computed at the contact point
        ...
        effective_shear_modulus = Young_modulus / \
            (4*(2 - Poisson_ratio)*(1+Poisson_ratio))

        damping_coeff = self.compute_damping_coefficient(
            equivalent_mass, equivalent_radius, total_overlap, effective_Young_modulus)
        normal_hertz = self.compute_normal_hertzian_force(
            equivalent_radius, total_overlap, effective_Young_modulus)
        tangential_hertz_stiffness = self.compute_tangential_hertzian_stiffness(
            equivalent_radius, total_overlap, effective_shear_modulus)

        box_volume = box_lengths[0] * box_lengths[1] * \
            box_lengths[2]  # volume of tricilinic box

        # Process contacts for each pair of ellipsoids (first particle)
        normal_force = np.einsum(
            'ij,ij->i', self.force, global_normals)[:, np.newaxis]*global_normals
        ...
        stress = self.compute_stress(
            centers1, centers2, computed_force, box_volume)
        shear_stress_normal = self.compute_stress(
            centers1, centers2, normal_force, box_volume)
        shear_stress_tangential = self.compute_stress(
            centers1, centers2, tangential_force, box_volume)
        ...
        (normal_hist_cont_point_global,
            ...
            (tangential_hist_cont_point_global1,
             tangential_hist_cont_point_global2),
    ...
    def process_contacts(self, centers1, orientations, normal_force, tangential_force,
        ...
        (normal_hist_cont_point_global, tangential_hist_cont_point_global,
         counts_cont_point_global) = self.bin_forces_by_xy_angle_contact_point_global(
            # normal_forces_global, tangential_forces_global)
            self.point, centers1, normal_force, tangential_force)
        ...
        normal_hist_global, counts_normal_hist_global = self.accumulate_force_histogram(
            normal_force)
        # normal_hist_global, counts_normal_hist_global = self.accumulate_force_histogram(normal_force)
        tangential_hist_global, counts_tangential_hist_global = self.accumulate_force_histogram(
            tangential_force)
    ...
    def bin_1d_histogram(self, angles_deg, physical_quantity, ang_range, num_bins=10):
        ...
        if physical_quantity.ndim == 1:
            ...
            # Compute magnitude for 3D physical_quantity
            magnitudes = np.linalg.norm(physical_quantity, axis=1)
    ...
    def compute_ellipsoid_angle_local(self, contact_points, ellipsoid_centers, orientations):
        ...
        center_to_contact_vector_local = np.einsum(
            'ikj,ik->ij', orientations, center_to_contact_vector_global)
        angles_rad = np.arccos(
            center_to_contact_vector_local[:, 2] / np.linalg.norm(center_to_contact_vector_local, axis=1))
    ...
    def assign_data(self, data):
        ...
        # If you need to keep the filtered periodic values
        self.periodic = data[:, 2]
    ...
    def match_contact_data_with_particles(self, coor, orientation, shapex, shapez, vel, omega, box_lengths, shear_rate):
        ...
        centers1[adjust_y,
                 1] -= np.sign(distances_y[adjust_y]) * box_lengths[1]
        vel1[adjust_y, 0] -= np.sign(distances_y[adjust_y]) * \
            shear_rate*box_lengths[1]
        centers1[adjust_y,
                 0] -= np.sign(distances_y[adjust_y]) * box_lengths[3]
        ...
        centers1[adjust_x,
                 0] -= np.sign(distances_x[adjust_x]) * box_lengths[0]
        ...
        centers1[adjust_z,
                 2] -= np.sign(distances_z[adjust_z]) * box_lengths[2]
        ...
        self.point[adjust_y,
                   1] -= np.sign(dist_contact_y[adjust_y]) * box_lengths[1]
        self.point[adjust_y,
                   0] -= np.sign(dist_contact_y[adjust_y]) * box_lengths[3]
        ...
        self.point[adjust_x,
                   0] -= np.sign(dist_contact_x[adjust_x]) * box_lengths[0]
        ...
        self.point[adjust_z,
                   2] -= np.sign(dist_contact_z[adjust_z]) * box_lengths[2]
    ...
    def compute_particle_force_and_torque(self, centers1, centers2):
        ...
        # Vector from center of particle 1 to contact point
        contact_vector1 = self.point - centers1
        # Vector from center of particle 2 to contact point
        contact_vector2 = self.point - centers2
        ...
        torque2 = np.cross(contact_vector2, -self.force)

        # Accumulate forces and torques for each particle using np.add.at to handle possible multiple contacts per particle
        # Add contact forces to particle 1
        np.add.at(total_force, self.id1, self.force)
        # Add torques to particle 1
        np.add.at(total_torque, self.id1, torque1)

        # Subtract contact forces for particle 2
        np.add.at(total_force, self.id2, -self.force)
        # Add torques to particle 2
        np.add.at(total_torque, self.id2, torque2)
    ...
    def integrate_half_angular(self, inertia, torque, orientation, omega, dt):
        ...
        gyroscopic_term = np.cross(omega_local, np.einsum(
            'ijk,ik->ij', inertia, omega_local))
        ...
        delta_omega_global = np.einsum(
            'ijk,ik->ij', orientation, delta_omega_local)
    ...
    def compute_normals(self, centers, contact_points, rotations, shapex, shapez, force, shear):
        ...
        """
        local_contact_points = np.einsum(
            'ikj,ik->ij', rotations, contact_points - centers)
        ...
        computed_global_normals = np.einsum(
            'ijk,ik->ij', rotations, local_normals)
    ...
    def compute_gaussian_curvature_coefficient(self, a, c, contact_vectors, orientations):
        ...
        local_contact_points = np.einsum(
            'ikj,ik->ij', orientations, contact_vectors)
    ...
    def compute_mean_curvature(self, shapex, shapez, contact_points, centers, orientations):
        ...
        # Translate the contact points to the local frame of the ellipsoids
        local_contact_points = np.einsum(
            'ikj,ik->ij', orientations, contact_points - centers)
        ...
        num = np.einsum('ij,ij->i', gradient,
                        np.einsum('ijk,ik->ij', hessian, gradient))
        num2 = np.einsum('ij,ij->i', gradient, gradient) * \
            np.einsum('ijj->i', hessian)
    ...
    def compute_normal_hertzian_force(self, radius_curvature_avg, overlap, effective_Young_modulus):
        ...
        """
        normal_force = 4/3 * effective_Young_modulus * \
            np.sqrt(radius_curvature_avg * overlap)*overlap  # always positive
    ...
    def compute_tangential_hertzian_stiffness(self, radius_curvature_avg, overlap, effective_shear_modulus):
        ...
        shear_stiffness = 8*effective_shear_modulus * \
            np.sqrt(radius_curvature_avg*overlap)
    ...
    def compute_damping_coefficient(self, equivalent_mass, radius_curvature_avg, overlap, effective_Young_modulus, restitution_coeff=0.1):
        ...
        beta = np.log(restitution_coeff)/np.sqrt(np.pi **
                                                 2 + (np.log(restitution_coeff))**2)
        # Damping coefficient
        damping_coefficients = -2*np.sqrt(5/6)*beta*np.sqrt(
            2*equivalent_mass*effective_Young_modulus*np.sqrt(radius_curvature_avg*overlap))
    ...
    def bin_forces_by_xy_angle_contact_point_global(self, contact_points, ellipsoid_centers, normal_forces, tangential_forces, num_bins=144):
        ...
        """
        center_to_contact_vector_global = contact_points[:,
                                                         :2] - ellipsoid_centers[:, :2]
        angles_rad = np.arctan2(
            center_to_contact_vector_global[:, 1], center_to_contact_vector_global[:, 0])
        angles_deg = np.degrees(angles_rad)
        angles_deg = np.mod(angles_deg + 360, 360)

        normal_hist, bin_counts = self.bin_1d_histogram(
            angles_deg, normal_forces, 360, num_bins)
        tangential_hist, _ = self.bin_1d_histogram(
            angles_deg, tangential_forces, 360, num_bins)
    ...
    def bin_forces_by_ellipsoid_angle(self, contact_points, ellipsoid_centers, normal_forces, tangential_forces, orientations, num_bins=10):
        ...
        """
        angles_deg = self.compute_ellipsoid_angle_local(
            contact_points, ellipsoid_centers, orientations)
        normal_hist, bin_counts = self.bin_1d_histogram(
            angles_deg, normal_forces, 90, num_bins)
        tangential_hist, _ = self.bin_1d_histogram(
            angles_deg, tangential_forces, 90, num_bins)
    ...
    def bin_forces_by_ellipsoid_angle_mixed(self, contact_points, ellipsoid_centers, normal_forces, tangential_forces, orientations, num_bins_theta=10, num_bins_phi=10):
        ...
        angles_rad = np.arccos(
            center_to_contact_vector_local[:, 2] / np.linalg.norm(center_to_contact_vector_local, axis=1))
        axis = orientations[:, 2, :] / \
            np.linalg.norm(orientations[:, 2, :], axis=1)[:, np.newaxis]
        angles_deg = np.degrees(angles_rad)
        ...
        x_ref = x_global - \
            np.einsum('ij,ij->i', x_global, axis)[:, np.newaxis]*axis
        x_ref = mask_sign[:, np.newaxis] * x_ref / \
            np.linalg.norm(x_ref, axis=1)[:, np.newaxis]

        # project global contact point vector onto plane perpendicular to symmetry axis
        contact_point_plane = center_to_contact_vector_global - \
            np.einsum('ij,ij->i', center_to_contact_vector_global,
                      axis)[:, np.newaxis]*axis
        # compute the angle between the projected contact point vector and the reference x-axis
        # Dot product with x_ref gives x-component
        x_components = np.einsum('ij,ij->i', contact_point_plane, x_ref)
        # Cross product gives z-component (the sine part)
        y_components = np.cross(x_ref, contact_point_plane)[:, 2]
        ...
        # Use np.add.at for vectorized binning
        np.add.at(normal_hist_flat, combined_bin_indices,
                  np.linalg.norm(normal_forces, axis=1))
        np.add.at(tangential_hist_flat, combined_bin_indices,
                  np.linalg.norm(tangential_forces, axis=1))
    ...
    def accumulate_force_histogram(self, forces, num_bins=144):
        ...
        hist, bin_counts = self.bin_1d_histogram(
            angles_deg, forces, 360, num_bins)
    ...
    def compute_and_bin_dissipation(self, contact_points, ellipsoid_centers_1, normals,
        ...
        # modulus of normal velocity
        normal_vr = np.einsum('ij,ij->i', relative_velocities, normals)

        computed_normal_force = (
            (-damping_coeff*normal_vr - normal_hertz)[:, np.newaxis])*normals

        tangential_vr = relative_velocities - normal_vr[:, np.newaxis]*normals
        coulomb_limit = self.cof * \
            np.linalg.norm(computed_normal_force, axis=1)

        computed_tangential_forces = kt[:, np.newaxis] * shear
        sliding_limit = np.where(np.linalg.norm(
            computed_tangential_forces, axis=1) >= 0.99*coulomb_limit)
        # compare the computed tangential force with the tangential force
        if self.cof != 0:
            diff_tangential = np.linalg.norm(
                computed_tangential_forces-tangential_forces, axis=1)/np.linalg.norm(computed_tangential_forces, axis=1)
        else:
            diff_tangential = np.zeros_like(computed_tangential_forces)

        # compute the mean tangential force for sliding contacts
        mean_tangential_force = np.mean(np.linalg.norm(
            tangential_forces[sliding_limit], axis=1))

        friction_dissipation = np.zeros_like(kt)
        friction_dissipation[sliding_limit] = 0.5*np.abs(np.einsum(
            'ij,ij->i', computed_tangential_forces[sliding_limit], tangential_vr[sliding_limit]))
        damping_dissipation = 0.5 * damping_coeff * normal_vr**2

        # Binning process
        angles_deg = self.compute_ellipsoid_angle_local(
            contact_points, ellipsoid_centers_1, orientations_1)
        normal_dissipation_hist, bin_counts = self.bin_1d_histogram(
            angles_deg, damping_dissipation, 90, num_bins)
        tangential_dissipation_hist, _ = self.bin_1d_histogram(
            angles_deg, friction_dissipation, 90, num_bins)
    ...
    def compute_particle_stress(self, centers1, centers2, forces, particle_mass, density=1000):
        ...
        np.add.at(particle_stress, self.id1, np.einsum(
            'ij,ik->ijk', contact_vector1, forces))
        np.add.at(particle_stress, self.id2, np.einsum(
            'ij,ik->ijk', contact_vector2, -forces))
    ...
    def plot_force_chains(self, centers1, centers2, step):
        ...
        min_thickness = 0.001
        max_thickness = 3
        thickness = np.interp(magnitudes_above_avg, (magnitudes_above_avg.min(
        ), magnitudes_above_avg.max()), (min_thickness, max_thickness))
        ...
        unit_forces = forces_above_avg / magnitudes_above_avg[:, np.newaxis]
        dot_products = np.dot(unit_forces, shear_axis)
        angles = np.arccos(np.clip(dot_products, -1.0, 1.0)) * \
            (180.0 / np.pi)  # Convert radians to degrees

        # Map angles to [0, 90] range
        ...
        sm = plt.cm.ScalarMappable(
            cmap=colormap, norm=plt.Normalize(vmin=-45, vmax=45))
        sm.set_array([])
        bar = plt.colorbar(
            sm, ax=ax, orientation='horizontal', shrink=0.8, aspect=10)
        bar.set_label(
            'Angle with Compression direction (degrees)', fontsize=20)
        azimuth_angle = 360/50*step
        ax.view_init(elev=0, azim=90)
        # Reduce padding around the plot (tight layout)
        plt.tight_layout()
        plt.show()
        plt.savefig('force_chains/ap' + str(self.ap) + '_cof_' +
                    str(self.cof) + '_I_' + str(self.I) + '_step_' + str(step) + '.png')
        plt.close()
    ...
    def check_plot_global_forces(self, forces):
        ...
        # Normalize forces to unit vectors
        directions = forces / np.linalg.norm(forces, axis=1)[:, np.newaxis]

        print(directions[:, 0] > 1)

        # Angle between force vector and X-axis
        forces_angle_with_x = np.arccos(directions[:, 0])
        # create the histogram

        plt.figure(figsize=(10, 6))
        plt.hist(forces_angle_with_x, bins=100,
    ...
    def compute_force_distribution(self):
        ...
        self.force_distribution_x = np.histogram(
            np.abs(self.force[:, 0]), bins=100)
        self.force_distribution_y = np.histogram(
            np.abs(self.force[:, 1]), bins=100)
        self.force_distribution_z = np.histogram(
            np.abs(self.force[:, 2]), bins=100)
        self.force_distribution = np.histogram(
            np.linalg.norm(self.force, axis=1), bins=100)

    def compute_force_tangential_distribution(self):
        """Compute the tangential force distribution"""
        self.force_tangential_distribution_x = np.histogram(
            self.force_tangential[:, 0], bins=100)
        self.force_tangential_distribution_y = np.histogram(
            self.force_tangential[:, 1], bins=100)
        self.force_tangential_distribution_z = np.histogram(
            self.force_tangential[:, 2], bins=100)
        self.force_tangential_distribution = np.histogram(
            np.linalg.norm(self.force_tangential, axis=1), bins=100)

    def compute_space_averages(self):
        """Compute the space averages"""
        force_normal_magnitude = np.linalg.norm(self.force_normal, axis=1)
        force_tangential_magnitude = np.linalg.norm(
            self.force_tangential, axis=1)
        self.force_normal_space_average = np.mean(force_normal_magnitude)
        self.force_tangential_space_average = np.mean(
            force_tangential_magnitude)
        ...
        try:
            ...
            raise AttributeError(
                "The contact angles have not been computed yet.")
    ...
    def store_data_tracked_grains(self, data, n_sampled_particles):
        ...
        data_sampled = np.zeros(
            (avg_max_contact_per_particle*n_sampled_particles, 10))
        ...
        for i in range(n_sampled_particles):
            ...
            for j in range(rows_plus.shape[0]):
                ...
                # index of the particle the contact referes to in the zeroth column
                ...
                data_sampled[count, 7:10] = rows_plus[j, 12:15]
                count += 1

            for j in range(rows_minus.shape[0]):
                data_sampled[count, 0] = i
                # contact point
                data_sampled[count, 1:4] = rows_minus[j, 37:22]
