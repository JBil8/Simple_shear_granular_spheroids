    ...
    if results:
        for key in results[0].keys():
            ...
            if key in histogram_sums:
                histogram_sums[key] = np.sum([res[key] for res in results], axis=0)
            elif key in ['trackedGrainsOrientation', 'trackedGrainsPosition', 'thetax', 'thetaz']:
                distributions[key] = np.concatenate([res[key] for res in results])
            elif key in ['vy_velocity', 'omegaz_velocity']:
                distributions[key] = np.stack([res[key] for res in results], axis=1)
            else:
