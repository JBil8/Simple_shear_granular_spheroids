...
class ReaderDump(DataReader):
    ...
    def filter_relevant_files(self, prefix='simple_shear_contact_data'):
        self.file_list = [filename for filename in self.file_list if filename.startswith(
            prefix) and filename[len(prefix):len(prefix)+1].isdigit() and filename.endswith('.dump')]
