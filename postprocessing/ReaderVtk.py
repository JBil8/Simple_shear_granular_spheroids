...
class ReaderVtk(DataReader):
    ...
    def get_initial_velocities_and_orientations(self):
        ...
        self.v0 = np.array(polydata.GetPointData().GetArray(3))[
            self.sorted_idxs, :][self.n_wall_atoms:, :]
        self.y0 = np.array(polydata.GetPoints().GetData())[
            self.sorted_idxs, :][self.n_wall_atoms:, 1]
        self.v_shearing = np.array(polydatapoints.GetArray("v"))[
            self.sorted_idxs, :][self.n_wall_atoms-1, 0]
        orientations = np.array(polydatapoints.GetArray("TENSOR"))[
            self.sorted_idxs, :].reshape(self.n_central_atoms, 3, 3)
    ...
    def get_particles_volume(self):
        ...
        polydata = reader.GetOutput()
        polydatapoints = polydata.GetPointData()
        shapex = np.array(polydatapoints.GetArray("shapex"))[
            self.n_wall_atoms:]
        shapey = np.array(polydatapoints.GetArray("shapey"))[
            self.n_wall_atoms:]
        shapez = np.array(polydatapoints.GetArray("shapez"))[
            self.n_wall_atoms:]
    ...
    def filter_relevant_files(self, prefix='shear_ellipsoids_', box=False):
        file_list = [filename for filename in self.file_list if filename.startswith(
            prefix) and filename[len(prefix):len(prefix)+1].isdigit() and filename.endswith('.vtk')]
        if box:
            prefix = prefix + 'boundingBox_'
            self.file_list_box = [filename for filename in self.file_list if filename.startswith(
                prefix) and filename[len(prefix):len(prefix)+1].isdigit() and filename.endswith('.vtk')]
        self.file_list = file_list

    def get_box_dimensions(self):
        reader = vtk.vtkRectilinearGridReader()
        name_first_file = re.sub(
            r'(\d+)', r'boundingBox_\1', self.file_list[0])
