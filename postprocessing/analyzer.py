...
class SimulationAnalyzer:
    ...
    def _initialize_processors(self):
        ...
        # VTK Reader
        self.vtk_reader = ReaderVtk(
            self.cof, self.ap, I=self.param, pressure=self.pressure)
        self.vtk_reader.read_data(
            (config.GLOBAL_PATH + "/"), 'simple_shear_', box=True)
        ...
        dump_reader = ReaderDump(
            self.cof, self.ap, I=self.param, pressure=self.pressure)
        dump_reader.read_data((config.GLOBAL_PATH + "/"),
                              'simple_shear_contact_data_')

        vtk_processor = ProcessorVtk(self.vtk_reader)
        dump_processor = ProcessorDump(
            dump_reader, self.vtk_reader.n_wall_atoms, self.vtk_reader.n_central_atoms)

        self.combined_processor = CombinedProcessor(
            vtk_processor, dump_processor, self.vtk_reader.file_list_box, self.shear_rate, dt_hertz)
    ...
    def _run_parallel_processing(self):
        ...
        print(
            f"Starting analysis from shear step index: {self.shear_one_index}")

        # Define the range of steps to process
        steps_to_process = range(self.shear_one_index,
                                 self.combined_processor.n_sim)
        self.n_sim_processed = len(steps_to_process)

        with multiprocessing.Pool(self.num_processes) as pool:
            results = pool.map(
                self.combined_processor.process_single_step, steps_to_process)
        return results

    def _post_process(self, parallel_results):
        """Aggregates parallel results and computes final metrics."""
        area_adjust, total_area = area_adjustment_ellipsoid(
            config.NUM_BINS_LOCAL, self.ap)
        ...
        auto_corr_vel = compute_autocorrelation_function(
            distributions['vy_velocity'])
        auto_corr_omega = compute_autocorrelation_function(
            distributions['omegaz_velocity'])
        ...
        avg_dat = dat_processor.compute_averages(
            self.shear_one_index / self.combined_processor.n_sim)
        avg_dat = dat_processor.compute_max_vx_diff(avg_dat)

        # Combine all results into a single dictionary
        self.results_data = {**averages, **
                             hist_avg, **pdfs, **avg_csv, **avg_dat}

        # Add final computed values
        self.results_data['shear_rate'] = self.shear_rate
        self.results_data['thetax_mean'] = compute_circular_mean(
            distributions['thetax'], config.NUM_BINS_ORIENTATION)
        self.results_data['thetaz_mean'] = compute_circular_mean(
            distributions['thetaz'], config.NUM_BINS_ORIENTATION)
        self.results_data['pdf_thetax'] = compute_pdf_orientation(
            distributions['thetax'], config.NUM_BINS_ORIENTATION)
        self.results_data['pdf_thetaz'] = compute_pdf_orientation(
            distributions['thetaz'], config.NUM_BINS_ORIENTATION)
