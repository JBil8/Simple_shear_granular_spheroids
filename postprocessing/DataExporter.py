...
class DataExporter:
    ...
    def export_with_pickle_obstructed(self, averages, msd):
        ...
        with open(self.directory + '/obstruction_shear_ap' + self.ap + '_cof_' + self.cof +
    ...
    def export_data(self, data_vtk, data_dump):
        ...
        strain = np.arange(
            1, data_vtk['theta_x'].shape[0]+1)*20/data_vtk['theta_x'].shape[0]
        ...
        np.savetxt(
            self.directory + '/simple_shear_ap' + self.ap + '_cof_' +
            self.cof + '_' + self.parameter + '_' + self.value + '.csv',
        ...
        np.savetxt(
            self.directory + '/simple_shear_ap' + self.ap + '_cof_' + self.cof +
            '_' + self.parameter + '_' + self.value + '_eulerian_velocities.csv',
    ...
    def export_force_distribution(self, force_normal_distribution, force_tangential_distribution):
        ...
        with open(self.directory + '/simple_shear_ap' + self.ap + '_cof_' + self.cof + '_' + self.parameter + '_' + self.value + '_force_normal.pkl', 'wb') as f:
            pkl.dump(force_normal_distribution, f)

        with open(self.directory + '/simple_shear_ap' + self.ap + '_cof_' + self.cof + '_' + self.parameter + '_' + self.value + '_force_tangential.pkl', 'wb') as f:
